...
Note: C and C++ are not the only languages that can be called by high-level languages. Go and Rust support exporting code in library format. This repository contains experimental support for calling Go functions in libfraction.go and Rust functions in libfraction.rs.
